...
const QUESTIONS = [
  ...
  },
  {
    ...
    "correct": [
      ...
      "Może prowadzić do problemów związanych z zależnościami między instrukcjami W pytaniu jest literówka: „infrastrukturami” — najpewniej chodziło o instrukcjami."
    ],
    "wrong": [
  ...
  },
  {
    ...
    "correct": [
      ...
      "pthread_mutex_init(&mutex); — z doprecyzowaniem: w praktyce poprawne wywołanie ma"
    ],
    "wrong": [
  ...
  },
  {
    ...
    "wrong": [
      "Równoległe zawsze jest szybsze od rozproszonego — nie zawsze. Wydajność zależy od problemu, komunikacji, synchronizacji, liczby maszyn, sieci i narzutów. Przetwarzanie równoległe może być szybsze, ale nie zawsze."
    ],
    "uncertain": [],
    "uncertainOpen": false,
  ...
  },
  {
    ...
    "extra": false,
    "type": "open",
    "question": "Wypisz zależności pojawiające się w kodzie: 1: y = 4 * sin(2 * 3.14); 2: x = 2 * i + z * y; 3: z = x * y;",
    "correct": [],
    "wrong": [],
    "uncertain": [],
    "uncertainOpen": false,
    "explanation": "Zależności w kodzie Nr linii, nr linii Zmienna Typ zależności 1 → 2 y RAW — Read After Write 1 → 3 y RAW — Read After Write 2 → 3 x RAW — Read After Write 2 → 3 z WAR — Write After Read Wyjaśnienie Zależność RAW — Read After Write RAW oznacza, że jedna instrukcja najpierw zapisuje zmienną, a późniejsza instrukcja ją odczytuje. Przykład z kodu: 1: y = 4 * sin(2 * 3.14); 2: x = 2 * i + z * y; Linia 1 zapisuje y, a linia 2 odczytuje y. Czyli: 1 → 2 na zmiennej y: RAW Podobnie: 2: x = 2 * i + z * y; 3: z = x * y; Linia 2 zapisuje x, a linia 3 odczytuje x. Czyli: 2 → 3 na zmiennej x: RAW Zależność WAR — Write After Read WAR oznacza, że jedna instrukcja najpierw odczytuje zmienną, a późniejsza instrukcja ją zapisuje. Przykład: 2: x = 2 * i + z * y; 3: z = x * y; Linia 2 odczytuje starą wartość z, a linia 3 zapisuje nową wartość do z. Czyli: 2 → 3 na zmiennej z: WAR\n\nNajwazniejsza intuicja:\nW tym kodzie najważniejsze są zależności RAW przez y i x, bo późniejsze instrukcje potrzebują\nwyników wcześniejszych, oraz zależność WAR na z, bo linia 2 musi odczytać stare z, zanim linia 3 je\nnadpisze."
  },
  {
    "id": 36,
    "source": "PRO-MAX",
    ...
    "extra": false,
    "type": "open",
    "question": "Jakie będą wartości zmiennych a, b i c w dowolnym wątku? Kod: int a = 3; int b = 2; int c = 1; #pragma omp parallel firstprivate(a) num_threads(4) { int b = a * c; #pragma omp barrier #pragma omp atomic c += 2; a += 3; b += 4 #pragma omp barrier }",
    "correct": [],
    "wrong": [],
    "uncertain": [],
    "uncertainOpen": false,
    "explanation": "Odpowiedź dla dowolnego wątku wewnątrz regionu równoległego a = 6 b = 7 c = 9 Wyjaśnienie Zmienna a #pragma omp parallel firstprivate(a) firstprivate(a) oznacza, że każdy wątek dostaje własną kopię a, ale z wartością początkową z oryginalnej zmiennej. Na początku w każdym wątku: a = 3 Potem każdy wątek wykonuje: a += 3; więc w każdym wątku: a = 6 Zmienna b Na zewnątrz mamy: int b = 2; ale wewnątrz regionu równoległego jest nowa lokalna zmienna: int b = a * c; Ta zmienna b jest osobna dla każdego wątku. Na początku: a = 3 c = 1 więc: b = 3 * 1 = 3 Potem każdy wątek wykonuje: b += 4; czyli: b = 3 + 4 = 7 Zmienna c Zmienna c nie jest oznaczona jako private, więc jest współdzielona między wszystkimi wątkami. Na początku: c = 1 Mamy 4 wątki: num_threads(4) Każdy wykonuje atomowo: #pragma omp atomic c += 2; Czyli łącznie: c = 1 + 4 * 2 = 9 Uwaga Po wyjściu z regionu równoległego, w kodzie sekwencyjnym wartości zmiennych zewnętrznych byłyby: a = 3 b = 2 c = 9 bo a było firstprivate, a wewnętrzne b zasłaniało zewnętrzne b.\n\nNajwazniejsza intuicja:\nW wątku: a jest prywatną kopią, b jest lokalne dla bloku, a c jest wspólne i zwiększane atomowo\nprzez wszystkie 4 wątki."
  },
  {
    "id": 37,
    "source": "PRO-MAX",
  ...
  },
  {
    ...
    "wrong": [
      "synchronized na metodzie statycznej synchronizuje dostęp do niej na poziomie instancji klasy — instancji.",
  ...
  },
  {
    ...
    "wrong": [
      ...
      "Ustala dostęp tylko dla wątku głównego — do kodu wykonywanego przez wątek główny służy",
  ...
  },
  {
    ...
    "wrong": [
      ...
      "Każdy proces wykonuje się w izolacji, bez możliwości komunikacji — nie, procesy lub wątki współbieżne mogą się komunikować i synchronizować. SISO, SIMO, MISO, MIMO to pojęcia znane raczej z systemów komunikacyjnych i antenowych, nie z taksonomii Flynna. SMR, NUMA, UMA, MPP dotyczą organizacji pamięci lub typów systemów wieloprocesorowych, ale nie są podstawowymi kategoriami Flynna. RISC, CISC, superskalarność, potokowość dotyczą architektury procesora i sposobu wykonywania instrukcji, ale nie są klasyfikacją Flynna.",
  ...
  },
  {
    ...
    "wrong": [
      "Wyłącznie na procedury synchroniczne, ponieważ nie istnieją metody asynchroniczne — MPI_Irecv."
    ],
    "uncertain": [],
    "uncertainOpen": false,
  ...
  },
  {
    ...
    "correct": [
      ...
      "Zarządzania / organizowania hierarchii procesów, np. w architekturze master-worker Z doprecyzowaniem: interkomunikator sam nie „zarządza” procesami automatycznie, ale umożliwia komunikację między grupami, co można wykorzystać w układzie master-worker."
    ],
    "wrong": [
  ...
  },
  {
    ...
    "extra": false,
    "type": "mc",
    "question": "Które dyrektywy OpenMP służą do synchronizacji wątków?",
    "correct": [
      "#pragma omp barrier",
      "#pragma omp critical",
      "#pragma omp atomic",
  ...
  },
  {
    ...
    "extra": false,
    "type": "open",
    "question": "Wartości zmiennych po zakończeniu pętli OpenMP. Kod z obrazka: int a = 1, b = 4, c = 5, d = 0, i; #pragma omp parallel for private(b) shared(c) lastprivate(d) num_threads(3) for (i = 0; i < 3; i++) { b += 1; 4 c += b; 7 9 d = omp_get_thread_num(); 2 }",
    "correct": [],
    "wrong": [],
    "uncertain": [],
    "uncertainOpen": false,
    "explanation": "Odpowiedź a = 1 b = 4 c = ? d = 2 Z doprecyzowaniem: d = 2 przy typowym statycznym podziale 3 iteracji na 3 wątki. Formalnie wartość d zależy od tego, który wątek wykona ostatnią logiczną iterację i = 2. Wyjaśnienie Zmienna a a nie jest modyfikowana w pętli. int a = 1; Dlatego po pętli: a = 1 Zmienna b b jest oznaczona jako: private(b) To znaczy, że każdy wątek ma własną kopię b. Zewnętrzna zmienna: b = 4 nie jest zmieniana przez pętlę. Dlatego po pętli: b = 4 Uwaga: prywatna kopia b w wątku nie jest automatycznie inicjalizowana wartością 4, bo użyto private, a nie firstprivate. Zmienna c c jest oznaczona jako: shared(c) Czyli wszystkie wątki modyfikują tę samą zmienną: c += b; Nie ma tutaj atomic, critical ani redukcji, więc występuje race condition. Dodatkowo b jako private ma nieokreśloną wartość początkową. Dlatego: c = ? Zmienna d d jest oznaczona jako: lastprivate(d) To znaczy, że po zakończeniu pętli zmienna zewnętrzna d dostaje wartość z ostatniej logicznej iteracji pętli, czyli z iteracji: i = 2 W tej iteracji wykonywane jest: d = omp_get_thread_num(); Przy 3 iteracjach i 3 wątkach typowo: i = 0 → wątek 0 i = 1 → wątek 1 i = 2 → wątek 2 więc: d = 2\n\nNajwazniejsza intuicja:\nprivate(b) zostawia zewnętrzne b bez zmian, shared(c) bez synchronizacji daje wynik losowy, a\nlastprivate(d) przepisuje wartość z ostatniej iteracji pętli."
  },
  {
    "id": 48,
    "source": "PRO-MAX",
  ...
  },
  {
    ...
    "extra": false,
    "type": "mc",
    "question": "Co można powiedzieć o sposobie przetwarzania i dostępu do zmiennej id? Kod używa: #pragma omp parallel private(id)",
    "correct": [
      "id jest prywatne dla każdego wątku, co oznacza, że każdy wątek ma swoją kopię",
      "Jeśli nie użyto by klauzuli private(id), to id byłoby współdzielone i powodowało błędy",
